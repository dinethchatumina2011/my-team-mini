...
const BOT_NAME_FANCY = 'GPT TEAM MINI';
...
const config = {
  ...
  CHANNEL_LINK: '',
  BOT_NAME: 'GLITCH PHANTOMS MINI',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: '(dev) xanz',
  IMAGE_PATH: 'https://files.catbox.moe/qg5i86.jpeg',
  BOT_FOOTER: 'POWERD BY GPT TEAM',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/qg5i86.jpeg' }
};
...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ හිරුන්  GLITCH PHANTOMS TEAM" කියලා.* User Message: ${q}
    `;
...
case 'chr': {
  ...
  if (!q.includes(',')) return await socket.sendMessage(sender, { text: "❌ Usage: .cre <channelJid/messageId>,<emoji>" }, { quoted: msg });
...
case 'apk': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'GPT TEAM';
...
│ 👾 *\`Sᴛᴀᴛᴜꜱ :\`* Online
│ 👻 *\`Bᴏᴛ :\`* ${botName}
│ 👨‍💻 *\`Oᴡɴᴇʀ :\`* ${config.OWNER_NAME || 'GPT TEAM'}
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || 'GPT TEAM MINI';
...
case 'owner': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '㋚GPT TEAM MINI𝙸';
...
case 'tiktokdl': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'GPT TEAM MINI';
...
case 'grouplist': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || BOT_NAME_FANCY || "GPT TEAM MINI";
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of GPT  Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:GPT@gmail.com\n' + // Email
...
case 'instagram': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'GPT TEAM MINI𝙸';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'GPT TEAM MINI'; // dynamic bot name
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: 'GPT TEAM MINI', activesession: activeSockets.size });
});
